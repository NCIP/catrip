...
public class SimpleLinkRenderer implements IGraphLinkRenderer {
    ...
    private static final Color colorLink = Color.BLUE;//new Color (230, 139, 44); // sanjeev
...
//        if (strokeWidth > 1)
//            gr.setStroke (new BasicStroke (strokeWidth, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
        GeneralPath line = new GeneralPath ();
        line.moveTo (xs[0] + 0.5f, ys[0] + 0.5f);
        for (int i = 1; i < length; i ++) {
            line.lineTo (xs[i] + 0.5f, ys[i] + 0.5f);
//            floatLine.x2 = xs[i] + 0.5f;
//            floatLine.y2 = ys[i] + 0.5f;
//            gr.draw (floatLine);
//            floatLine.x1 = floatLine.x2;
//            floatLine.y1 = floatLine.y2;
        }
        gr.draw (line);
        if (! Double.isNaN (direction)) {
