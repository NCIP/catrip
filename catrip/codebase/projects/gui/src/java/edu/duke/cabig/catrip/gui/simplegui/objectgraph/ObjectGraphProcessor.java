...
public class ObjectGraphProcessor {
    ...
    public static void main(String[] args) {
        ObjectGraphProcessor ogp = new ObjectGraphProcessor("C:\\Development\\simplegui\\sample.xml");
        List objs = ogp.getAvialbleTargetObjectsToAssociateInRemoteServices("caTissueCore");
        Iterator itr = objs.iterator();

        while (itr.hasNext()) {
            GraphObject obj = (GraphObject)itr.next();
            System.out.println(obj.getClassName());
            System.out.println(obj.getServiceName());
            List l1 = ogp.getAssociatedObjects(obj.getClassName(),obj.getServiceName(),true);
            Iterator itr1 = l1.iterator();
            while (itr1.hasNext()) {
                obj = (GraphObject)itr1.next();
                System.out.println("   " + obj.getClassName());
                List l = obj.getAssociationPathWRTTargetObject();
                for (int i=0;i<l.size();i++) {
                    GraphAssociation as = (GraphAssociation)l.get(i);
                    System.out.println("              " + as.getClassName() + "  " + as.getRoleName());
                }                
            }
        }        
        
/*
        List objs = ogp.getAllTragetObjects();
        Iterator objItr = objs.iterator();
        while (objItr.hasNext()) {
            GraphObject gobj = (GraphObject)objItr.next();
            System.out.println(gobj.getClassName());
            System.out.println(gobj.getDisplaybleAttributes());
            
            List outPath = gobj.getForeignAssociationOutboundPath();
            Iterator outItr = outPath.iterator();
            while (outItr.hasNext()){
                GraphAssociation assoc = (GraphAssociation)outItr.next();
                System.out.println("out bound path : " + assoc.getClassName() + " " + assoc.getRoleName());
            }
            
        }
        /*
        System.out.println("++++++++++++++++");
        List list = ogp.getAssociatedObjects("edu.duke.catrip.cae.domain.general.Participant","caTissueCAE");//,"edu.wustl.catissuecore.domainobject.ClinicalReport");
        ...
        while (itr.hasNext()) {
            ...
            List l = obj.getAssociationPathToTargetObject();
